...
class ModelEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.is_final_layer():
            ...
            fps, avg_util, max_util, available_resources, resources_total = self.final_action_wall()
            ...
            reward = self.reward(acc, avg_util, max_util, available_resources, resources_total)
    ...
    def reward(self, acc, avg_util, max_util, available_resources, resources_total):
        ...
        # Increase LUTs weight in case any of the other weights are zero
        if weight_BRAM == 0:
            weight_LUT += 0.125
        elif weight_DSP == 0:
            weight_LUT += 0.125
        ...
        return acc_weight * acc_weighted + weight_BRAM * resources_BRAM + weight_LUT * resources_LUT + weight_DSP * resources_DSP
    ...
    def final_action_wall(self):
        while True:
            ...
            model, cycles, avg_util, max_util, available_resources, resources_total = set_folding(model, self.args.output_dir, self.args.board, self.args.freq, self.args.target_fps)
    ...
    def maximum_fps(self):
        ...
        model = create_dataflow_partition(model)
        model = specialize_layers(model, self.args.fpga_part)
        model, cycles, _, max_util, available_resources, resources_total = set_folding(model, self.args.output_dir, self.args.board, self.args.freq, self.args.target_fps)
        fps = self.args.freq * 10**6 / cycles
    ...
    def get_acc(self):
        return 0 # self.cur_acc

    def get_avg_util(self):
        return 0 # self.cur_avg_util  # Assuming `avg_util` is stored as an attribute